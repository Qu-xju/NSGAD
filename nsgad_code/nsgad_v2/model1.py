...
class Discriminator(nn.Module):
    ...
    def forward(self, c, h_pl,sub_flag,blance):
        scs = []  #
        # positive
        if sub_flag == 1:
            temp1 = h_pl - blance*c
...
class Model_bwg(nn.Module):
    def __init__(self, n_in, n_h, activation, negsamp_round, readout,d):
        super(Model_bwg, self).__init__()
        self.read_mode = readout
        # self.gcn = GCN(n_in, n_h, activation)
        self.bwg = My_BWGNN(n_in, n_h, num_classes=2, d=d, batch=False)
    ...
    def forward(self, seq1, adj, spec_flag='bwg',sub_flag=1,blance=1.0,sparse=False):
        if spec_flag=='gcn':
        ...
        ret = self.disc(c, h_mv,sub_flag,blance)

        return ret  ,f_1
    
class Model_low(nn.Module):
    def __init__(self, n_in, n_h, activation, negsamp_round, readout):
        super(Model_low, self).__init__()
        self.read_mode = readout
        self.gcn = GCN(n_in, n_h, activation)
    ...
    def forward(self, seq1, adj, sub_flag=1,blance=1.0,sparse=False):
        # if spec_flag=='gcn':
        h_1 = self.gcn(seq1, adj, sparse)
        # if spec_flag=='bwg':
        #     h_1 = self.bwg(seq1,adj)
